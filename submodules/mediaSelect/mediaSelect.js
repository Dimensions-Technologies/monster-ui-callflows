...
define(function(require) {
	...
	var mediaSelect = {
		...
		subscribe: {
			'callflows.mediaSelect.render': 'mediaSelectRender'
		},
		...
		mediaSelectFormatData: function(args) {
			var self = this,
				getDefaultEntities = function(args) {
					var orderedDefaultEntityTypes = [
						...
						defaultEntitiesPerType = {
							none: {
								id: 'none',
								name: _.get(args, 'noneLabel', self.i18n.active().callflows.mediaSelect.noneLabel),
								shouldRender: _.get(args, 'hasNone', true)
							},
							silence: {
								id: 'silence_stream://300000',
								name: self.i18n.active().callflows.mediaSelect.silence,
								shouldRender: _.get(args, 'hasSilence', true)
							},
							shoutcast: {
								id: 'shoutcast',
								name: self.i18n.active().callflows.mediaSelect.shoutcastURL,
				...
				defaultData = {
					...
					options: [],
					name: 'mediaId',
					uploadLabel: self.i18n.active().callflows.upload,
					label: args.hasOwnProperty('label') ? args.label : self.i18n.active().callflows.mediaSelect.defaultLabel,
					isShoutcast: false,
		...
		mediaSelectGetSkinnedTemplate: function(args, formattedData) {
			...
			if (skin === 'default') {
				...
				template = $(self.getTemplate({
					...
					data: _.merge({}, formattedData, {
						...
						ttsInfo: self.getTemplate({
							name: '!' + self.i18n.active().callflows.mediaSelect.textToSpeech.info + ' ' + self.i18n.active().callflows.mediaSelect.textToSpeech.language + ttsVoice,
							data: {
				...
				monster.ui.chosen(template.find('.media-dropdown'), {
					width: '460px'
				});
				...
				monster.ui.validate(mediaForm, {
					...
					messages: {
						mediaId: self.i18n.active().callflows.mediaSelect.invalidMedia
					}
				});
		...
		mediaSelectBindCommon: function(template, mediaToUpload, callbackAfterSave, mimeTypes) {
			...
			template.find('.upload-input').fileUpload({
				...
				error: function(errors) {
					if (errors.hasOwnProperty('size') && errors.size.length > 0) {
						monster.ui.alert(self.i18n.active().callflows.mediaSelect.fileTooBigAlert);
					}
			...
			template.find('.upload-submit').on('click', function() {
				...
				template.find('.upload-response').hide();
				...
				if (mediaToUpload) {
							...
							self.callApi({
								...
								success: function(data, status) {
									...
									if (media) {
										...
										template.find('.upload-response.success').show();
									}
									callbackAfterSave && callbackAfterSave(media);
								},
								error: function(data, status) {
									template.find('.upload-response.error').show();
									self.callApi({
					...
					monster.ui.alert(self.i18n.active().callflows.mediaSelect.emptyUploadAlert);
				}
			});
			...
			template.find('.upload-toggle').on('click', function() {
				if (args.dragableUpload) {
					popup = monster.ui.dialog(popupTemplate, {
						title: self.i18n.active().callflows.mediaSelect.uploadNewMedia,
						width: '400px',
						position: ['center', 20]
					});
		...
		mediaSelectUpdateTTSMedia: function(args, callback) {
			...
			if (args.selectedMedia.id && args.selectedMedia.media_source === 'tts') {
				self.callApi({
					...
					error: function() {
						monster.ui.alert(self.i18n.active().callflows.mediaSelect.textToSpeech.errors.update);
				...
				return self.callApi({
					...
					error: function() {
						monster.ui.alert(self.i18n.active().callflows.mediaSelect.textToSpeech.errors.create);
						callback && callback(null);
					}
				});
			}
