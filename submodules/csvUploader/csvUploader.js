define(function(require) {
	...
	return {
		subscribe: {
			'callflows.csvUploader.renderPopup': 'csvUploaderRender'
		},
		...
		csvUploaderRender: function(args) {
			var self = this,
				...
				$popup = monster.ui.dialog($template, {
					title: _.get(args, 'title', self.i18n.active().callflows.csvUploader.defaultTitle),
					onClose: function() {
		...
		csvUploaderBindEvents: function(args) {
				...
				successHandler = function(files) {
					...
					if (!_.isEmpty(prasedFile.errors)) {
						return errorHandler(
							_.get(
								...
								'message',
								self.i18n.active().callflows.csvUploader.invalidFile
							)
						);
					}
					var expectedHeader = args.header,
						...
						invalidRowLines = _
							.chain([
								...
								invalidRows.length > 5 ? self.i18n.active().callflows.csvUploader.andMore : []
							])
							.flatten()
							.join(', ')
							.value();
					...
					if (!_.isEmpty(invalidRows)) {
						return errorHandler(self.getTemplate({
							name: '!' + self.i18n.active().callflows.csvUploader.invalidRows,
							data: {
			...
			$form.find('.file-upload-input').fileUpload({
				dataFormat: 'text',
				filesList: [self.i18n.active().callflows.csvUploader.noFile],
				inputOnly: true,
				wrapperClass: 'file-upload input-append',
				btnClass: 'monster-button',
				btnText: self.i18n.active().callflows.csvUploader.chooseFile,
